...
class Model_PLCOm2012(Model):
    ...
    def train_model(self, X, y, name=None):
        print("Pre-existing model. No training performed.")
        return None
    
    def predict(self, X, name=None):
        return self.PLCOm2012(X)
    
    def PLCOm2012(self, X):
        for c in self.column_requirements:
            if c not in X.columns:
                print(
                    f"Missing one or more of required columns, including {c}." +
                    f"\nRequired columns: {self.column_requirements}"
                )
                return None
        return X.apply(self.PLCOm2012_single, axis=1)

    def PLCOm2012_single(self, X):
        age = X['age']
        race = X['race']
